"""Tests for MarkItemReadyCommand handler."""
...
from tabb.domain.exceptions.business import OrderItemNotFoundError
...
class TestMarkItemReadyHandler:
    @pytest.fixture()
    def uow(self):
        return _mock_uow()

    @pytest.fixture()
    def handler(self, uow) -> MarkItemReadyHandler:
        return MarkItemReadyHandler(uow, _id_generator())

    async def test_marks_item_ready(self, handler, uow) -> None:
        await handler.handle(MarkItemReadyCommand(order_id="o-1", order_item_id="oi-1"))

        uow.order_repository.save.assert_awaited_once()
        saved_order = uow.order_repository.save.call_args[0][0]
        assert saved_order.items[0].status == OrderItemStatus.READY
    ...
    async def test_order_not_found_raises(self) -> None:
        ...
        handler = MarkItemReadyHandler(uow, _id_generator())

        with pytest.raises(OrderNotFoundError):
            await handler.handle(
                MarkItemReadyCommand(order_id="o-1", order_item_id="oi-1")
            )

    async def test_item_not_found_raises(self, handler) -> None:
        with pytest.raises(OrderItemNotFoundError):
            await handler.handle(
                MarkItemReadyCommand(order_id="o-1", order_item_id="nonexistent")
            )
